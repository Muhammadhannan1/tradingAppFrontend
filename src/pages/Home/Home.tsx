  ...
  const fetchDataAndUpdateState = async () => {
    try {
      const apiData = await fetchData(
        `http://52.0.177.84:6100/api/v1/coinData/getData?categoryType=${categotyType.toLowerCase()}&timeFrame=${timeFrame}`
      );
      console.log(apiData);
  ...
  return (
    <>
      ...
      >
        <DropDown
          list={categoryList}
          currentValue={categotyType}
          changeFunction={handleCategoryChange}
        />
        <DropDown
          list={TimeFrameList}
          currentValue={timeFrame}
          changeFunction={handleTimeChange}
        />
      </Box>
